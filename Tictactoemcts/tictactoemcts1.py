...
class Board:
        ...
        for dir in direction: # ROW, COLUMN등 한 방향
            ...
            while True:
                ...
                if x < 0 or x >= self.board_size or y < 0 or y >= self.board_size:
                    break
                # 같은 모양이 아니면 탈출
                elif self.board[y][x] != player or self.board[y][x] == 0:
                    break
                else:
    ...
    def is_win(self, player):
        ...
        for direction in directions:
            if self.get_count(player, direction):
                return True
        return False

    def is_draw(self):
        if self.get_cell_count() == 9:
            return True
        return False
...
class Node:
    def __init__(self, player, board, parent = None):
        ...
        self.visit_number = 0
        self.child = []
        self.empty_positions = self.board.get_empty_positions()

    def add_child(self):
        pos = self.empty_positions.pop()
        node = Node(3 - self.player, self.board, self)
        node.board.set_cell(pos, self.player)
        node.empty_positions.remove(pos)
    ...
    def is_finish(self, player, board):
        return board.is_win(player) or board.is_draw()
    ...
    def random_play(self, board, player):
        while not self.is_finish(player, board):
            player = 3 - player
            pos = random.choice(board.get_empty_positions())
            board.set_cell(pos, player)
        return self.get_result(board, player)
...
class Monte_carlo_tree_search:
    ...
    def mcts(self, player, iterate_number):
        node = Node(player, self.board)
    ...
    def expansion(self):
        player = self.node.board.get_cell(self.node.board.current_position)
        if not self.node.is_finish(player, self.node.board):
            self.node = self.node.add_child()

    def simulation(self):
        player = self.node.board.get_cell(self.node.board.current_position)
        return self.node.random_play(copy.deepcopy(self.node.board), player)
